...
def all_input(wildcards):
    ...
    for sample in samples.index:
        wanted_input.extend (
            expand (
                [
                    ...
                    "results/noderad/representatives/{sample}.xml.gz"
                ],
                sample = sample
            )
        )
        ...
        if config["graph-intermediate-steps"]["edit-distance-graph"]["activate"]:
            wanted_input.extend(expand(["results/noderad/graph/{sample}.pdf"], sample = sample))
        if config["graph-intermediate-steps"]["connected-components"]["graph"]["activate-xml"]:
            wanted_input.extend (
                expand (
                    [
                        "results/noderad/connected_components/{sample}.all_components.xml.gz"
                    ],
                    sample = sample
                )
            )
        if config["graph-intermediate-steps"]["connected-components"]["graph"]["activate-figure"]:
            wanted_input.extend (
                expand (
                    [
                        "results/noderad/connected_components/{sample}.all_components.pdf"
                    ],
                    sample = sample
                )
            )
        if config["graph-intermediate-steps"]["connected-components"]["subgraphes"]["activate"]:
            wanted_input.extend (
                directory(
                    expand (
                        [
                            "results/noderad/connected_components/subgraphes/{sample}"
                        ],
                        sample = sample
                    )
                )
        ...
        if config["graph-intermediate-steps"]["ilp-optimized-graph"]["activate"]:
            wanted_input.extend(expand(["results/noderad/representatives/{sample}.pdf"], sample = sample))
    return wanted_input
